<?php
session_start();
$connect = mysqli_connect("localhost", "root", "123456789", "rush00");
?>
<!doctype html>
<html>
<head>
...
<div class="container" style="width:60%;">
    ...
    if(mysqli_num_rows($result) > 0)
    {
        while($row = mysqli_fetch_array($result))
        {
            ...
            <input type="number" name="quantity" min="1" max="50" class="form-control" value="1" style="width: 60%">
            <input type="hidden" name="hidden_name" value="<?php echo $row["name"]; ?>">
            <input type="hidden" name="hidden_price" value="<?php echo $row["price"]; ?>">
            <input type="submit" name="add" style="margin-top:5px;" value="Add to Cart">
            <button type="submit" formaction="./index.php?page=basket">Go to cart</button>
            </div>
            </form>
            </div>
            <?php
